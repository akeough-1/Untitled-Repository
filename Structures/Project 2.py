...
class Struct_Frame():
    ...
    def __init__(self, E:float, h:float, A:float, I:float):
        self.E = E
        self.h = h
        self.A = A
        self.I = I
    ...
    def solve(self):
        ...
        self.loads = np.zeros((num_nodes,1)) # applied loading
        self.free_nodes = [] # nodes with no displacement constraint
        self.fixed_nodes = [] # constrained nodes
        ...
        for joint in self.joints:
            ...
            if joint.DoF == 0:
                ...
                self.fixed_nodes.append(node_num+2)
            elif joint.DoF == 1:
                self.fixed_nodes.append(node_num)
                self.fixed_nodes.append(node_num+1)
                self.free_nodes.append(node_num+2)
            elif joint.DoF == 2:
            ...
            self.loads[node_num] = joint.force_x
            self.loads[node_num+1] = joint.force_y
            self.loads[node_num+2] = joint.moment

            # the loop handles 1 joint (3 nodes) at a time so update node index to reflect
            node_num += 3

        self.S = np.zeros((num_nodes,num_nodes)) # global stiffness matrix of the entire frame
        ...
        self.S = np.delete(np.delete(self.S, self.fixed_nodes, axis=0), self.fixed_nodes, axis=1)
        self.loads = np.delete(self.loads, self.fixed_nodes, axis=0)

        # some linear algebra to solve for d in P = Sd
        self.d = np.linalg.solve(self.S, self.loads)
    ...
    def plot_displacement(self):
        ...
        class Frame_Plot(tk.Frame):
            def __init__(self, master:tk, frame:Struct_Frame):
                ...
                self.ax.set_ylabel("Y dimension (in)")
                self.ax.set_xlabel("X dimension (in)")
                self.ax.set_title("Structure shape change")
    ...
    def print_values(self):
        member_properties = ["k","K","u","Q","F"]

        for property in member_properties:
            for i in range(len(self.members)):
                member = self.members[i]
                df = pd.DataFrame(getattr(member, property))
                df.index = member.index + 1

                if property == "k" or property == "K":
                    df.columns = member.index + 1
        ...
        for i in range(len(self.members)):
            member = self.members[i]
            print(f"Axial Stress {i+1}")
            print(f"{member.a_stress} ksi")
            print("")

            print(f"Axial Strain {i+1}")
            print(f"{member.a_strain} micron")
            print("")

        print("P")
        df = pd.DataFrame(self.loads)
        df.index = np.array(self.free_nodes) + 1
        print(df)
        print("")

        print("S")
        df = pd.DataFrame(np.round(self.S,2))
        df.index = np.array(self.free_nodes) + 1
        df.columns = np.array(self.free_nodes) + 1
        print(df)
        print("")
...
class Member():
    ...
    def member_disp(self, E:float, I:float):
        """Function to calculate the coordinates of the displacement of the beam"""
        def mac(x,a):
            """Function to replicate the Macaulay function for discontinuity"""
            if x <= a:
                return 0
            elif x > a:
                return x - a

        # Discontinuity constants are defined by boundary conditions
        C1 = float(self.u[2,0])
        C2 = float(self.u[1,0])
        ...
        for i in range(num_points):
            ...
            self.trans_disp[i] += (self.Q[1,0]/6*mac(x[i],0)**3 - self.Q[2,0]/2*mac(x[i],0)**2 + self.Q[4,0]/6*mac(x[i],self.length)**3 - self.Q[5,0]/2*mac(x[i],self.length)**2)/I/E

            # The constants are already calculated in terms of I and E so do not include them "inside the parentheses"
            self.trans_disp[i] += C1*x[i] + C2

        # axial displacement is just the basic axial equation from Mechanics of Materials
        self.axial_disp = x*self.a_stress/E + self.u[0]

        # do a little trig to transform the deflection to the global axes
...
ex2 = Struct_Frame(E=E, h=h, A=A, I=I)
