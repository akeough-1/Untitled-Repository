...
class Magnetometer(InertialSensor):
    """ Class for the magnetometer, developed by Anderson Keough
        for Cyclone Rocketry
    ...
    measurement_range : float, tuple
        The measurement range of the sensor in µT.
    resolution : float
        The resolution of the sensor in µT/LSB.
    noise_density : float, list
        The noise density of the sensor in µT/√Hz.
    noise_variance : float, list
        The variance of the noise of the sensor in (µT)^2.
    random_walk_density : float, list
        The random walk density of the sensor in µT/√Hz.
    random_walk_variance : float, list
        The variance of the random walk of the sensor in (µT)^2.
    constant_bias : float, list
        The constant bias of the sensor in µT.
    operating_temperature : float
        The operating temperature of the sensor in Kelvin.
    temperature_bias : float, list
        The temperature bias of the sensor in µT/K.
    ...
    _random_walk_drift : Vector
        The random walk drift of the sensor in µT.
    measurement : float
    ...
    def __init__(
        ...
        measurement_range : float, tuple, optional
            The measurement range of the sensor in the µT. If a float, the
        ...
        resolution : float, optional
            The resolution of the sensor in µT/LSB. Default is 0, meaning no
            quantization is applied.
        noise_density : float, list, optional
            The noise density of the sensor for a Gaussian white noise in µT/√Hz.
        ...
        noise_variance : float, list, optional
            The noise variance of the sensor for a Gaussian white noise in (µT)^2.
            Default is 1, meaning the noise is normally distributed with a
            standard deviation of 1 µT. If a float or int is given, the same
        ...
        random_walk_density : float, list, optional
            The random walk of the sensor for a Gaussian random walk in µT/√Hz.
        ...
        random_walk_variance : float, list, optional
            ...
            (µT)^2. Default is 1, meaning the noise is normally distributed
            with a standard deviation of 1 µT. If a float or int is given,
        ...
        constant_bias : float, list, optional
            The constant bias of the sensor in µT. Default is 0, meaning no
        ...
        temperature_bias : float, list, optional
            The temperature bias of the sensor in µT/K. Default is 0,
    ...
    def measure(self, time, **kwargs):
        """Measure the magnetic field relative to the orientation of the rocket
        ...
        date_str = str(kwargs["environment"].datetime_date).partition(" ")[0]
        year = decimal_year(date_str)
        lat = kwargs["environment"].latitude
        lon = kwargs["environment"].longitude
        elev = float(kwargs["environment"].elevation)
        alt = (u[3] + elev)/1000 # convert to km for magnetic field model

        # Initialize the model
        wmm = WMMv2()

        north_intens = wmm.get_north_intensity(lat,lon,year,alt)/1000 # convert from nT to µT
        east_intens = wmm.get_east_intensity(lat,lon,year,alt)/1000
        vert_intens = wmm.get_vertical_intensity(lat,lon,year,alt)/-1000 # change down + to up +

        B = Vector([north_intens, east_intens, vert_intens])

        # Transform to sensor frame
        inertial_to_sensor = self._total_rotation_matrix @ Matrix.transformation(
            u[6:10]
        )
        B = inertial_to_sensor @ B
